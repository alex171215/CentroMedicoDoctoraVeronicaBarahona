...
export const salud = {
    ...
    async inicializar() {
        ...
        const citaDestacada = sessionStorage.getItem('cita_destacada');
        if (citaDestacada) {
            sessionStorage.removeItem('cita_destacada');
            // Pequeño delay para asegurar que el DOM de la lista esté renderizado
            setTimeout(() => this.verDetalleCita(String(citaDestacada).trim()), 300);
        }
    },
    ...
    async verDetalleCita(id) {
        ...
        const horaStr = cita.hora ? String(cita.hora).slice(0, 5) : '00:00'; // HH:MM seguro
        const fechaCitaISO = cita.fecha && /^\d{4}-\d{2}-\d{2}$/.test(cita.fecha)
            ? `${cita.fecha}T${horaStr}`
            : null;
        const fechaCitaMs = fechaCitaISO ? new Date(fechaCitaISO).getTime() : NaN;
        const horasRestantes = isNaN(fechaCitaMs)
            ? Infinity // sin fecha parseable → no bloquear
            : (fechaCitaMs - Date.now()) / (1000 * 60 * 60);
        const bloqueadoPor24h = !esCancelada && horasRestantes < 24;
        ...
        const accionesHtml = !esCancelada ? `
                ...
                ? `<p class="cita-aviso-24h" role="alert" style="font-size:0.85rem;color:#c0392b;margin:12px 0 8px;display:flex;align-items:center;gap:6px;">
                        ...
                        Ya no es posible modificar o cancelar esta cita (Solo se pueden modificar o cancelar 24 horas antes de la cita).
                       </p>`
        ...
        detBody.innerHTML = `
                ...
                <div class="salud-det__row">
                    ...
                    <span class="salud-det__val">${escapeHtml((cita.paciente || cita.nombres || '').trim() || 'No especificado')}</span>
                </div>
                ${cita.tipo ? `
                <div class="salud-det__row">
    ...
    verDetalleReceta(id) {
        ...
        const meds = r.medicamentos.map(m => {
            return `
                <div class="salud-med">
                    <p class="salud-med__nombre">${escapeHtml(m.nombre)}</p>
