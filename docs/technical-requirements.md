...
3. **Limpieza en Evento Input (Heurística #9):** Al detectar cualquier `input` en un campo, el sistema DEBE destruir de inmediato el Tooltip Flotante y la clase `.input-rechazado` activos en ese control, sin esperar al TTL ni al `blur`. El temporizador de 2,5 s queda únicamente como fallback si el usuario deja de escribir manteniendo el foco.
...
## TR-99: Ciclo de Vida Reactivo y Mitigación de Errores en Formularios (Heurística #9)
...
## TR-128: Unificación de Identidad Nacional y Validación de Módulo 10 (H4, H5)
